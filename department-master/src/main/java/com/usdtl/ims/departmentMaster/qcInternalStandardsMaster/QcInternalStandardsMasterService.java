...
@Service
@AllArgsConstructor
public class QcInternalStandardsMasterService {
    private QcInternalStandardsMasterRepository repository;
    ...
    public QcInternalStandardsMasterEntity createItem(QcInternalStandardsMasterRequest request) {
        QcInternalStandardsMasterEntity newItem = QcInternalStandardsMasterEntity.builder()
    ...
    public Page<QcInternalStandardsMasterEntity> getExtractionsExperienceItemsByPage(Integer page) {
        PageRequest pageRequest = PageRequest.of(page, 10);
    ...
    public Page<DepartmentMasterResponse> getExperienceItemsByPage(Integer page) {
        ...
        PageRequest pageRequest = PageRequest.of(page, 10);
        Long qualityItemsCount = repository.count();
        List<QcInternalStandardsMasterEntity> extractionsItems = repository.findAll(pageRequest).getContent();
        ...
        return new PageImpl<>(departmentMasterItems, pageRequest, qualityItemsCount);
    }

    public QcInternalStandardsMasterEntity getItemById(Integer id) throws NotFoundException {
        QcInternalStandardsMasterEntity item = repository.findById(id).orElseThrow(() ->  new NotFoundException("Item associated with id: " + id + " not found"));
        return item;
    }

//    public void updateOrderQuantity() {
//        List<QualityExperienceEntity> items = (List<QualityExperienceEntity>) repository.findAll();
//        items.forEach((item) -> {
