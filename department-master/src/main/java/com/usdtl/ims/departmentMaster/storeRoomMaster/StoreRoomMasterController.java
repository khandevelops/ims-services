...
public class StoreRoomMasterController {
    ...
    @GetMapping("list/raw")
    public Page<StoreRoomMasterEntity> getExtractionsExperienceItemsByPage(@RequestParam Integer page) {
        return service.getExtractionsExperienceItemsByPage(page);
    }
    @GetMapping(path = "{id}")
    public  ResponseEntity<StoreRoomMasterEntity> getItemById(@PathVariable(value = "id") Integer id) throws NotFoundException {
        return new ResponseEntity<>(service.getItemById(id), HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<StoreRoomMasterEntity> createItem(@RequestBody StoreRoomMasterRequest request) {
