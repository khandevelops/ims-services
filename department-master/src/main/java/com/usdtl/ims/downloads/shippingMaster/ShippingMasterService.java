package com.usdtl.ims.downloads.shippingMaster;
...
public class ShippingMasterService {
    ...
    public ShippingMasterEntity createItem(ShippingMasterRequest request) {
        ShippingMasterEntity newItem = ShippingMasterEntity.builder()
    ...
    public Page<ShippingMasterEntity> getExtractionsExperienceItemsByPage(Integer page) {
        PageRequest pageRequest = PageRequest.of(page, 10);
    ...
    public Page<DepartmentMasterResponse> getExperienceItemsByPage(Integer page) {
        ...
        PageRequest pageRequest = PageRequest.of(page, 10);
        Long shippingItemsCount = repository.count();
        List<ShippingMasterEntity> extractionsItems = repository.findAll(pageRequest).getContent();
        ...
        return new PageImpl<>(departmentMasterItems, pageRequest, shippingItemsCount);
    }

    public ShippingMasterEntity getItemById(Integer id) throws NotFoundException {
        ShippingMasterEntity item = repository.findById(id).orElseThrow(() ->  new NotFoundException("Item associated with id: " + id + " not found"));
        return item;
    }

//    public void updateOrderQuantity() {
//        List<ShippingExperienceEntity> items = (List<ShippingExperienceEntity>) repository.findAll();
//        items.forEach((item) -> {
...
//    }
    ...
    private Integer getTotalQuantity(Integer id) {
        MasterDepartmentResponse response = client.getMasterShippingItemById(id);
        return response.shippingItems().stream().mapToInt(DepartmentResponse::quantity).sum();
    }
}