package com.usdtl.ims.download.department.screeningMaster;
...
public class ScreeningMasterService {
    ...
    public ScreeningMasterEntity createItem(ScreeningMasterRequest request) {
        ScreeningMasterEntity newItem = ScreeningMasterEntity.builder()
    ...
    public Page<ScreeningMasterEntity> getExtractionsExperienceItemsByPage(Integer page) {
        PageRequest pageRequest = PageRequest.of(page, 10);
    ...
    public Page<DepartmentMasterResponse> getExperienceItemsByPage(Integer page) {
        ...
        PageRequest pageRequest = PageRequest.of(page, 10);
        Long screeningItemsCount = repository.count();
        List<ScreeningMasterEntity> extractionsItems = repository.findAll(pageRequest).getContent();
        ...
        return new PageImpl<>(departmentMasterItems, pageRequest, screeningItemsCount);
    }

    public ScreeningMasterEntity getItemById(Integer id) throws NotFoundException {
        ScreeningMasterEntity item = repository.findById(id).orElseThrow(() ->  new NotFoundException("Item associated with id: " + id + " not found"));
        return item;
    }

//    public void updateOrderQuantity() {
//        List<ScreeningExperienceEntity> items = (List<ScreeningExperienceEntity>) repository.findAll();
//        items.forEach((item) -> {
