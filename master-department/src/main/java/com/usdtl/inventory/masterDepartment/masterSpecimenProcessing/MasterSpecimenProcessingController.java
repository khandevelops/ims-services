...
public class MasterSpecimenProcessingController {
    ...
    @GetMapping(path = "{id}")
    public  ResponseEntity<MasterSpecimenProcessingEntity> getItem(@PathVariable(value = "id") Integer id) throws NotFoundException {
        return new ResponseEntity<>(service.getItem(id), HttpStatus.OK);
    }

    @GetMapping("filter")
    public Page<MasterSpecimenProcessingEntity> filterItems(@RequestParam String keyword, @RequestParam Integer page) {
        return service.filterItems(keyword, page);
    }

    @GetMapping("sort")
    public Page<MasterSpecimenProcessingEntity> sortItems(@RequestParam Integer page, @RequestParam String column, @RequestParam String direction) {
        return service.sorItems(page, column, direction);
    }

    @GetMapping("list")
    public Page<MasterSpecimenProcessingEntity> getItems(@RequestParam Integer page) {
