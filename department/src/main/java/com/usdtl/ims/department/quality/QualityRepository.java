...
public interface QualityRepository extends PagingAndSortingRepository<QualityEntity, Integer> {
    @Query(value = "SELECT new com.usdtl.ims.clients.responseClass.DepartmentMasterResponseTest" +
            ...
            " FROM QualityEntity AS e INNER JOIN MasterEntity AS m ON m.id = e.item_id GROUP BY e.item_id, e.usage_level, e.min_quantity, e.max_quantity"
    )
    ...
    @Query(value = "SELECT new com.usdtl.ims.clients.responseClass.DepartmentMasterResponseTest" +
            ...
            " FROM QualityEntity AS e RIGHT JOIN MasterEntity AS m ON m.id = e.item_id"
    )
