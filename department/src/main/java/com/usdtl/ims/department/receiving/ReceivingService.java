...
public class ReceivingService {
    ...
    public List<ReceivingEntity> updateDepartmentItems(List<ReceivingEntity> requestItems) {
        List<ReceivingEntity> updateItems = new ArrayList<>();
        requestItems.forEach(departmentItem -> {
            ReceivingEntity item = repository.findById(departmentItem.getId()).orElseThrow(() -> new NotFoundException("Item associated with id: " + departmentItem.getId() + " not found"));
