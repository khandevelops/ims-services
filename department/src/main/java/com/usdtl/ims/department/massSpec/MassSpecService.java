...
public class MassSpecService {
    ...
    public List<MassSpecEntity> updateDepartmentItems(List<MassSpecEntity> requestItems) {
        List<MassSpecEntity> updateItems = new ArrayList<>();
        requestItems.forEach(departmentItem -> {
            MassSpecEntity item = repository.findById(departmentItem.getId()).orElseThrow(() -> new NotFoundException("Item associated with id: " + departmentItem.getId() + " not found"));
