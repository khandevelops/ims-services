...
public interface ExtractionsRepository extends PagingAndSortingRepository<ExtractionsEntity, Integer> {
    @Query(value = "SELECT new com.usdtl.ims.clients.responseClass.DepartmentMasterResponseTest" +
            ...
            " FROM ExtractionsEntity AS e INNER JOIN MasterEntity AS m ON m.id = e.item_id GROUP BY e.item_id, e.usage_level, e.min_quantity, e.max_quantity"
    )
    ...
    @Query(value = "SELECT new com.usdtl.ims.clients.responseClass.DepartmentMasterResponseTest" +
            ...
            " FROM ExtractionsEntity AS e RIGHT JOIN MasterEntity AS m ON m.id = e.item_id"
    )
