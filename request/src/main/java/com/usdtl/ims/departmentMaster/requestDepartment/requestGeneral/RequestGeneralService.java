...
public class RequestGeneralService {
    ...
    public Page<RequestDepartmentResponse> getRequestItemsByPage(Integer page) {
        PageRequest pageRequest = PageRequest.of(page, 10);
        List<RequestDepartmentResponse> requestTransformedItems = new ArrayList<>();
        Long requestItemCount = repository.count();
        List<RequestGeneralEntity> requestMasterItems = repository.findAll(pageRequest).getContent();
